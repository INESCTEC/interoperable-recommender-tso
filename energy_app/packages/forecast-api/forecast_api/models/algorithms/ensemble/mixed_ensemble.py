...
class MixEns(BaseEstimator, RegressorMixin, ModelClass):
    """
    Mixed Ensemble with Decision Tree

    Parameters
    ----------
    estimators : list of (string, estimator) tuples
        Invoking the ``fit`` method on the ``estimatot`` will fit clones
    ...
    def __init__(self, estimators, clf_class=tree.DecisionTreeClassifier(),
                 alpha=0.1,
                 method='hard',
                 n_jobs=1,
                 B=KFold(n_splits=3),
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if n_has_quantiles < len(self.estimators):
            raise ValueError('At least on of the provided estimators does '
                             'not have quantiles as parameters!')
        ...
        self.weights = np.array([
            MixEns.__compute_candidate_best(y_hat[i], y,
        ...
        if self.verbose > 0:
            print(pd.DataFrame(self.weights.T).
                  reset_index().
                  groupby(0).agg('count'))

        # fit a decision tree
        self.clf_class.fit(X, self.weights.T)
    ...
    def predict(self, X, **kwargs):
        ...
        **kwargs: for compatibility with ModelClass
        ...
        pred_out = np.zeros((pred.shape[1], pred.shape[0]))
        # n_est = pred.shape[2]
        if self.method == 'hard':
            p_cl = self.clf_class.predict(X)
            for i in np.unique(p_cl).astype(int):
                mask = (p_cl == i)
                # pred_out
                pred_out += np.multiply(pred.T[i], mask)
        elif self.method == 'soft':
            p_cl = np.asarray(self.clf_class.predict_proba(X))
            pred_out = np.multiply(p_cl, pred).sum(axis=2).T
        return pred_out
    ...
    def get_params(self, deep=True):
        ...
        if not deep:
            return super(MixEns, self).get_params(deep=False)
        else:
            out = super(MixEns, self).get_params(deep=False)
            ...
            for key, value in six.iteritems(
                    self.clf_class.get_params(deep=True)):
                out['%s__%s' % (name, key)] = value
    ...
    @staticmethod
    def __compute_candidate_best(y_hat, y, tau=0.5, alpha=0.1):
        ...
        w = np.array(
            [_pinball_loss_q(real=y, pred=y_hat[:, i], tau=tau) for i in
             range(n_est)])
        w = np.exp(-alpha * w)
        # mask = (np.abs(w.max(axis=0) - w.min(axis=0)) < np.finfo(
        #     np.float32).eps)
        cl = np.argmax(w, axis=0)
        # cl[mask] = -1
        return cl
