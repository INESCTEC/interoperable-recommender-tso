...
def linear_quantile_regression(dataset: pd.DataFrame,
    ...
    logger.debug(f"[GenForecast:{country_code}]] In process ...")
    logger.debug(f"[GenForecast:{country_code}]] Forecast range: "
    ...
    # Feature engineering:
    log_msg_ = f"[GenForecast:{country_code}]] Creating inputs ..."
    ...
    log_msg_ = f"[GenForecast:{country_code}]] Train X/y split ..."
    logger.debug(log_msg_)
    # -- Train:
    ...
    log_msg_ = f"[GenForecast:{country_code}]] Operational X/y split ..."
    logger.debug(log_msg_)
    ...
    log_msg_ = f"[GenForecast:{country_code}]] Forecasting ..."
    logger.debug(log_msg_)
    ...
    logger.debug(f"[GenForecast:{country_code}]] In process ... Complete!")
    ...
    if nr_complete_hours < forecasts.shape[0]:
        logger.warning(f"[GenForecast:{country_code}] "
                       f"Number of complete hours: {nr_complete_hours} "
                       f"out of {forecasts.shape[0]}")
    else:
        logger.info(f"[GenForecast:{country_code}] "
                    f"Number of complete hours: {nr_complete_hours} "
                    f"out of {forecasts.shape[0]}")
